 ...
 *     1 假设 用户 点击获取后货地址的提示框 确定 authSetting scope.address
 *      scope 值 true 直接调用 获取收货地址
 *     2 假设 用户 从来没有调用过收货地址的api
 *      scope undefined 直接调用 获取收货地址
 *     3 假设 用户 点击获取后货地址的提示框 取消
 *      scope 值 false
 *      1 诱导用户 自己 打开 授权设置页面 当用户重新给予 获取地址权限的时候
 *      2 获取收货地址
 *     4 把获取到的收货地址 存入到 本地缓存中
 * 2 页面加载完毕
 *  0 onLoad onShow
 *  1 获取本地存储中的地址数据
 *  2 把数据 设置给data中的一个变量
...
Page({
    ...
    onShow: function () {
        ...
        // 只要 有一个回调函数返回值为false 那么不再循环执行， 直接返回false
